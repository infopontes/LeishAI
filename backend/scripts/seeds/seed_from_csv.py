...
def map_clinical_data(row: dict) -> dict:
    ...
    """
    lesion_mapping = { "Alterada": "Leves/Moderadas", "Aumentados": "Leves/Moderadas", "Grave": "Graves" }
...
def seed_from_csv(db: Session) -> None:
    ...
    if not default_owner:
        ...
        default_owner_in = owner_schema.OwnerCreate( name=default_owner_name, phone="+5586994244568", address="Rua A", neighborhood="Reis Veloso", city="Parnaíba", state="PI" )
    ...
    try:
        with open(csv_file_path, mode='r', encoding='latin-1') as csvfile:
            ...
            logger.info(f"Found {total_rows} rows in CSV file. Starting import...")
            ...
            for i, row in enumerate(reader):
                ...
                if not animal:
                    animal_in = animal_schema.AnimalCreate( name=row.get('nome') or "Nome Não Informado", original_id=animal_original_id, sex=row.get('sexo'), owner_id=owner.id, breed_id=breed.id )
                    animal = crud_animal.create_animal(db, animal=animal_in)

                # Criação do Atendimento
                clinical_data = map_clinical_data(row)
                assessment_in = assessment_schema.AssessmentCreate( animal_id=animal.id, **clinical_data )
