...
def seed_roles_and_users(db: Session) -> None:
    ...
    if not admin_user:
        ...
        new_admin = crud_user.create_user(db, user=user_in) # É criado como inativo
        new_admin.role_id = admin_role.id
        new_admin.is_active = True  # 👈 **CORREÇÃO CRÍTICA: Ativação explícita**
        db.commit()
        logger.info(f"Created AND activated admin user: {admin_email}")
    else:
        logger.info(f"Admin user '{admin_email}' already exists. Skipping.")

    # Criar Utilizador Veterinário
    ...
    if not vet_user:
        ...
        new_vet = crud_user.create_user(db, user=user_in) # É criado como inativo
        new_vet.is_active = True # 👈 **CORREÇÃO CRÍTICA: Ativação explícita**
        db.commit()
